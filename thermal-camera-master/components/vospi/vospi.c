...
{
  //ESP_LOGI(TAG, "In transfer_segment");
  ...
  while ((segment->packets[0].id & 0x0f00) == 0x0f00) {
    ...
    //ESP_LOGI(TAG, "segment_packet[0].id %x", segment->packets[0].id);
  }
  //ESP_LOGI(TAG, "segment_packet[0].id %x", segment->packets[0].id);
...
int sync_and_transfer_frame(vospi_frame_t* frame)
{
  ...
  while (1) {
      ...
      ESP_LOGI(TAG, "TTT bits were: %x P20 Num: %d", ttt_bits, packet_20_num);
      if (ttt_bits == 1) {
        break;
      }
  }
